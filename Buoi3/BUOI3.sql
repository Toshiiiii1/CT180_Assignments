--CREATE USER buoi3 IDENTIFIED BY "buoi3";
--GRANT ALL PRIVILEGES TO buoi3;
...
-- Cau 5:
SELECT SACH.NXB FROM
SACH INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT
...
-- Cau 7:
SELECT DOCGIA.HO, DOCGIA.TEN FROM
MUON INNER JOIN DOCGIA ON MUON.ND = DOCGIA.ND
WHERE MUON.NGAYMUON >= '9-15-2007' AND MUON.NGAYMUON <= '9-20-2007';

-- Cau 8:
SELECT DISTINCT SACH.NXB FROM
TACPHAM INNER JOIN SACH ON TACPHAM.NT = SACH.NT
WHERE TACPHAM.TUA = 'Germinal ';

-- Cau 9:
SELECT DOCGIA.HO, DOCGIA.TEN FROM
((TACPHAM INNER JOIN SACH ON TACPHAM.NT = SACH.NT)
INNER JOIN MUON ON SACH.NS = MUON.NS)
INNER JOIN DOCGIA ON DOCGIA.ND = MUON.ND
WHERE TACPHAM.TUA = 'Po?e';
...
-- Cau 11:
SELECT TACPHAM.TUA, DOCGIA.HO, DOCGIA.TEN FROM
((MUON INNER JOIN DOCGIA ON MUON.ND = DOCGIA.ND)
INNER JOIN SACH ON MUON.NS = SACH.NS)
INNER JOIN TACPHAM ON TACPHAM.NT = SACH.NT
...
INNER JOIN SACH ON MUON.NS = SACH.NS)
INNER JOIN TACPHAM ON TACPHAM.NT = SACH.NT
WHERE MUON.NGAYMUON >= '1-1-2007' AND MUON.NGAYMUON <= '12-31-2007';

-- Cau 15:
SELECT COUNT(NT) SOLUONGTACPHAM FROM TACPHAM;

-- Cau 16:
SELECT TACPHAM.TUA, COUNT(*) SO_QUYEN_SACH FROM
TACPHAM INNER JOIN SACH ON SACH.NT = TACPHAM.NT
GROUP BY TACPHAM.TUA
HAVING COUNT(*) > 1;

-- Cau 17:
SELECT TACGIA, COUNT(TACGIA) SOLUONGTACPHAM
FROM TACPHAM
GROUP BY TACGIA;

-- Cau 18:
SELECT TACPHAM.TUA, COUNT(*) SO_LUONG_SACH FROM 
TACPHAM INNER JOIN SACH ON TACPHAM.NT = SACH.NT
GROUP BY TACPHAM.TUA;

-- Cau 19:
SELECT DOCGIA.HO, DOCGIA.TEN, DOCGIA.ND, COUNT(DOCGIA.ND) SO_LAN_MUON FROM
MUON INNER JOIN DOCGIA ON MUON.ND = DOCGIA.ND
GROUP BY DOCGIA.HO, DOCGIA.TEN, DOCGIA.ND;
...
-- Cau 21:
-- Cach 1:
SELECT TACPHAM.TUA, SOLUONGSACH.SOLUONG FROM
TACPHAM INNER JOIN 
(SELECT NT, COUNT(NT) SOLUONG FROM SACH GROUP BY NT ORDER BY SOLUONG DESC) SOLUONGSACH
ON TACPHAM.NT = SOLUONGSACH.NT
WHERE ROWNUM = 1;
...
-- Cau 22:
SELECT NXB, SO_LUONG_SACH_XUAT_BAN FROM
    (SELECT NXB, COUNT(NXB) SO_LUONG_SACH_XUAT_BAN FROM SACH
    GROUP BY NXB
    ORDER BY SO_LUONG_SACH_XUAT_BAN DESC)
WHERE ROWNUM = 1;

-- Cau 23:
SELECT COUNT(*) FROM 
  (SELECT TACPHAM.TACGIA, COUNT(TACPHAM.TACGIA) SO_LUONG_TAC_PHAM FROM 
  TACPHAM INNER JOIN SACH ON TACPHAM.NT = SACH.NT
  GROUP BY TACPHAM.TACGIA
  HAVING COUNT(TACPHAM.TACGIA) > 1);

-- Cau 24:
SELECT TUA, SO_LAN_MUON FROM
  (SELECT TACPHAM.TUA, COUNT(TACPHAM.TUA) SO_LAN_MUON FROM
  (SACH INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT)
  INNER JOIN MUON ON MUON.NS = SACH.NS
  GROUP BY TACPHAM.TUA
...
SELECT * FROM
  ...
  INNER JOIN MUON ON MUON.NS = SACH.NS
  GROUP BY TACPHAM.TUA
  ORDER BY SO_LAN_MUON);
WHERE SO_LAN_MUON < 2;

-- Cau 26:
SELECT HO, TEN, SO_LAN_MUON FROM
  (SELECT DOCGIA.HO, DOCGIA.TEN, DOCGIA.ND, COUNT(DOCGIA.ND) SO_LAN_MUON FROM
  ((SACH INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT)
  INNER JOIN MUON ON MUON.NS = SACH.NS)
  INNER JOIN DOCGIA ON DOCGIA.ND = MUON.ND
...
-- Cau 28:
SELECT TACPHAM.TUA FROM
(SACH LEFT JOIN MUON ON SACH.NS = MUON.NS)
INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT
WHERE ND IS NULL;
...
-- Cau 29:
SELECT TACPHAM.TUA, SACH.NS FROM
(SACH LEFT JOIN MUON ON SACH.NS = MUON.NS)
INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT
...
SELECT * FROM SACH INNER JOIN TACPHAM ON SACH.NT = TACPHAM.NT;
