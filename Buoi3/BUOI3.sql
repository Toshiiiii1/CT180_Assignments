...
TACPHAM INNER JOIN 
(SELECT NT, COUNT(NT) SOLUONG FROM SACH GROUP BY NT) SOLUONGSACH
ON TACPHAM.NT = SOLUONGSACH.NT
WHERE SOLUONGSACH.SOLUONG >= 2;
...
-- Cau 19:
SELECT DOCGIA.HO, DOCGIA.TEN, DANHSACHMUON.SOLANMUON FROM
DOCGIA INNER JOIN (SELECT ND, COUNT(ND) SOLANMUON FROM MUON GROUP BY ND) DANHSACHMUON
ON DOCGIA.ND = DANHSACHMUON.ND;
