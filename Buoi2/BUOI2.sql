...
-- Cau 5: thuc hien noi ket tu nhien giua ba bang lan luot la: LOP + PHIEUTHU + HOCVIEN
SELECT HOCVIEN.MAHV, HOCVIEN.TENHV FROM
HOCVIEN INNER JOIN (PHIEUTHU INNER JOIN LOP ON PHIEUTHU.MALOP = LOP.MALOP) ON HOCVIEN.MAHV = PHIEUTHU.MAHV
WHERE MAKH = 'K001';
...
WHERE LOP.MALOP = 'L001' AND LOAILOP.MALOAI = 'LL001';

-- Cau 12:
SELECT * FROM
(CHUONGTRINH INNER JOIN LOAILOP ON CHUONGTRINH.MACT = LOAILOP.MACT)
INNER JOIN LOP ON LOAILOP.MALOAI = LOP.MALOAI -- tim ra nhung lop thuoc chuong trinh TATQ
WHERE CHUONGTRINH.MACT = 'CT001';

-- Cau 13: lop 1?
SELECT * FROM
((PHIEUTHU INNER JOIN LOP ON PHIEUTHU.MALOP = LOP.MALOP) -- lay ra nhung phieu thu thuoc lop 1
INNER JOIN LOAILOP ON LOP.MALOAI = LOAILOP.MALOAI) -- lay ra nhung phieu thu thuoc lop 1 thuoc TAA1
WHERE LOP.MALOP = 'L003' AND LOAILOP.MALOAI = 'LL002';

-- Cau 14:
SELECT HOCVIEN.TENHV, MONHOC.TENMH, DIEM.DIEM FROM
(((PHIEUTHU INNER JOIN LOP ON PHIEUTHU.MALOP = LOP.MALOP)
INNER JOIN HOCVIEN ON PHIEUTHU.MAHV = HOCVIEN.MAHV)
INNER JOIN DIEM ON DIEM.MAHV = HOCVIEN.MAHV AND LOP.MALOP = LOP.MALOP)
INNER JOIN MONHOC ON MONHOC.MAMH = DIEM.MAMH
WHERE LOP.MAKH = 'K001'
ORDER BY HOCVIEN.TENHV;
