CREATE USER buoi4 IDENTIFIED BY "buoi4";
GRANT ALL PRIVILEGES TO buoi4;
...
-- Cau 3:
SELECT * FROM ktrucsu
WHERE NAMS_KTS = '1956' AND HOTEN_KTS LIKE 'le%';
...
-- Cau 6:
SELECT DISTINCT CHUTHAU.TEN_THAU, CHUTHAU.DCHI_THAU FROM
((KTRUCSU INNER JOIN THIETKE ON KTRUCSU.HOTEN_KTS = THIETKE.HOTEN_KTS)
INNER JOIN CGTRINH ON CGTRINH.STT_CTR = THIETKE.STT_CTR)
INNER JOIN CHUTHAU ON CHUTHAU.TEN_THAU = CGTRINH.TEN_THAU
WHERE KTRUCSU.HOTEN_KTS = 'le kim dung' AND CGTRINH.TINH_THANH = 'can tho';

-- Cau 7:
SELECT KTRUCSU.NOI_TN FROM
(CGTRINH INNER JOIN THIETKE ON CGTRINH.STT_CTR = THIETKE.STT_CTR)
INNER JOIN KTRUCSU ON KTRUCSU.HOTEN_KTS = THIETKE.HOTEN_KTS
WHERE CGTRINH.TEN_CTR = 'khach san quoc te' AND CGTRINH.TINH_THANH = 'can tho';

-- Cau 8:
SELECT CONGNHAN.HOTEN_CN, CONGNHAN.NAMS_CN, CONGNHAN.NAM_VAO_N FROM
(CGTRINH INNER JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR)
INNER JOIN CONGNHAN ON THAMGIA.HOTEN_CN = CONGNHAN.HOTEN_CN
WHERE CGTRINH.TEN_THAU = 'le van son' AND (CONGNHAN.CH_MON = 'han' OR CONGNHAN.CH_MON = 'dien');

-- Cau 9:
SELECT THAMGIA.HOTEN_CN FROM
CGTRINH INNER JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR
WHERE CGTRINH.TEN_CTR = 'khach san quoc te' AND CGTRINH.TINH_THANH = 'can tho' AND THAMGIA.NGAY_TGIA BETWEEN 'DEC-15-1994' AND 'DEC-31-1994';

-- Cau 10:
SELECT KTRUCSU.HOTEN_KTS, KTRUCSU.NAMS_KTS FROM
(CGTRINH INNER JOIN THIETKE ON CGTRINH.STT_CTR = THIETKE.STT_CTR)
INNER JOIN KTRUCSU ON THIETKE.HOTEN_KTS = KTRUCSU.HOTEN_KTS
WHERE CGTRINH.KINH_PHI > '400'
GROUP BY KTRUCSU.HOTEN_KTS, KTRUCSU.NAMS_KTS
HAVING COUNT(THIETKE.HOTEN_KTS) >= 1;

-- Cau 11:
SELECT CONGNHAN.HOTEN_CN, CONGNHAN.CH_MON FROM
((CGTRINH INNER JOIN THIETKE ON CGTRINH.STT_CTR = THIETKE.STT_CTR)
INNER JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR)
INNER JOIN CONGNHAN ON THAMGIA.HOTEN_CN = CONGNHAN.HOTEN_CN
...
-- Cau 14:
SELECT TEN_THAU, SUM(KINH_PHI) TONG_KINH_PHI FROM CGTRINH
GROUP BY TEN_THAU;

-- Cau 15:
SELECT CHUTHAU.TEN_THAU, CHUTHAU.DCHI_THAU FROM
CGTRINH INNER JOIN CHUTHAU ON CGTRINH.TEN_THAU = CHUTHAU.TEN_THAU
WHERE CGTRINH.KINH_PHI = (SELECT MIN(KINH_PHI) FROM CGTRINH);
...
-- Cau 18:
SELECT HOTEN_KTS, COUNT(*) SO_LUONG_THIET_KE FROM THIETKE
GROUP BY HOTEN_KTS; 

-- Cau 19:
SELECT STT_CTR, COUNT(HOTEN_CN) TONG_SO_CONG_NHAN FROM THAMGIA
GROUP BY STT_CTR;
...
WHERE THAMGIA.HOTEN_CN = 'nguyen hong van' AND 'DEC-18-1994' BETWEEN THAMGIA.NGAY_TGIA AND THAMGIA.NGAY_TGIA + SO_NGAY;

-- Cau 23:
SELECT THIETKE.HOTEN_KTS FROM
CGTRINH INNER JOIN THIETKE ON CGTRINH.STT_CTR = THIETKE.STT_CTR
WHERE CGTRINH.TEN_THAU = 'phong dich vu so xd'
INTERSECT
SELECT THIETKE.HOTEN_KTS FROM
CGTRINH INNER JOIN THIETKE ON CGTRINH.STT_CTR = THIETKE.STT_CTR
WHERE CGTRINH.TEN_THAU = 'cty xd so 6';

-- Cau 24:
SELECT THAMGIA.HOTEN_CN FROM
CGTRINH JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR
WHERE CGTRINH.TINH_THANH = 'can tho'
MINUS
SELECT THAMGIA.HOTEN_CN FROM
CGTRINH JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR
WHERE CGTRINH.TINH_THANH = 'vinh long';
...
-- Cau 29:
SELECT A.TEN_THAU, B.TEN_THAU, A.TINH_THANH, A.STT_CTR, B.STT_CTR
FROM CGTRINH A JOIN CGTRINH B ON A.TINH_THANH = B.TINH_THANH
WHERE A.STT_CTR != B.STT_CTR AND A.TEN_THAU != B.TEN_THAU;
