...
CGTRINH INNER JOIN THAMGIA ON CGTRINH.STT_CTR = THAMGIA.STT_CTR
GROUP BY CGTRINH.STT_CTR, CGTRINH.TEN_CTR, CGTRINH.DIACHI_CTR
HAVING COUNT(THAMGIA.HOTEN_CN) = (SELECT MAX(TONG_SO_CONG_NHAN) FROM (SELECT STT_CTR, COUNT(HOTEN_CN) TONG_SO_CONG_NHAN FROM THAMGIA GROUP BY STT_CTR))
;
